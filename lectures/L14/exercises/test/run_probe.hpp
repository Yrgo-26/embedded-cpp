...
namespace probe
{
/** Index of a pipe's read end, as returned by pipe(). */
constexpr std::size_t readEnd{0U};

/** Index of a pipe's write end, as returned by pipe(). */
constexpr std::size_t writeEnd{1U};

/** How many file descriptors pipe() fills in, and how many are polled. */
constexpr std::size_t pipeEnds{2U};

/** Exit status the child uses when it was not killed first. */
constexpr int childSuccess{0};

/** Size of the buffer a single read() from the report pipe fills. */
constexpr std::size_t readBufferSize{4096U};

/** Size of the buffer a single read() from the result pipe fills. */
constexpr std::size_t resultBufferSize{256U};

/** No signal ended the child; it ran to the end. */
constexpr int noSignal{0};

/** Buffer size handed to std::setvbuf: 0 with _IONBF means unbuffered. */
constexpr std::size_t unbufferedSize{0U};

/** Added to the poll timeout so a zero-millisecond wait still polls once. */
constexpr int minimumPollTime_ms{1};

/** The byte ProbeSerial::read() reports as received. */
constexpr std::uint8_t receivedByte{0x5AU};
...
public:
    ...
    {
        const char report[pipeEnds]{'W', static_cast<char>(byte)};
        [[maybe_unused]] const auto written = ::write(myFd, report, sizeof(report));
    }

    bool read(std::uint8_t& byte) noexcept override
    {
        [[maybe_unused]] const auto written = ::write(myFd, "R", sizeof("R") - 1U);
        if (myReceives) { byte = receivedByte; }
        return myReceives;
    }
...
inline Report runProbe(const std::function<void(int)>& body, const std::chrono::milliseconds watch)
{
    ...
    if (0 == child)
    {
        ...
        dup2(output[writeEnd], STDOUT_FILENO);
        // Unbuffered, so nothing run() prints is lost when the child is killed.
        std::setvbuf(stdout, nullptr, _IONBF, unbufferedSize);
        body(calls[writeEnd]);
        _exit(childSuccess);
    }
    close(calls[writeEnd]);
    ...
    {
        pollfd fds[pipeEnds]{{calls[readEnd], POLLIN, 0}, {output[readEnd], POLLIN, 0}};
        const auto left = std::chrono::duration_cast<std::chrono::milliseconds>(
            deadline - std::chrono::steady_clock::now());
        if (0 < poll(fds, pipeEnds, static_cast<int>(left.count()) + minimumPollTime_ms))
        {
            if (0 != (fds[readEnd].revents & POLLIN)) { collect(calls[readEnd], raw); }
            if (0 != (fds[writeEnd].revents & POLLIN)) { collect(output[readEnd], report.printed); }
        }
    }
    ...
    while (0 < collect(calls[readEnd], raw)) {}
 ...
 */
[[nodiscard]] inline std::string inChild(const std::function<std::string()>& body, int& crashed)
{
    int result[pipeEnds]{};
    crashed = noSignal;
    ...
    if (0 == child)
    {
        close(result[readEnd]);
        const std::string text{body()};
        [[maybe_unused]] const auto written = ::write(result[writeEnd], text.data(), text.size());
        _exit(childSuccess);
    }
    close(result[writeEnd]);
    std::string text{};
    char buffer[resultBufferSize];
    for (auto count = ::read(result[readEnd], buffer, sizeof(buffer)); 0 < count;
         count      = ::read(result[readEnd], buffer, sizeof(buffer)))
    {
 ...
 */
[[nodiscard]] inline std::string firstCalls(const Report& report)
{
    ...
    for (std::size_t i{}; i < writes; ++i)
    {
        if (report.written[i] != static_cast<std::uint8_t>(i))
        {
            ...
            message << "write number " << (i + 1U) << " sent "
                    << static_cast<std::uint32_t>(report.written[i]) << ", expected " << i
