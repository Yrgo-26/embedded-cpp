/**
 * @file Tests for Övningsset 1: the factory interface, the two factories and the logic class,
 ...
 */
template<typename T>
[[nodiscard]] constexpr bool isNonCopyableAndNonMovable() noexcept
{
...
/**
 * @brief Övning 1.1: the factory interface is still abstract, with a virtual noexcept
 *        destructor.
 */
...
/**
 * @brief Övning 1.1: serial() still takes two pins and is noexcept, and now returns a
...
/**
 * @brief Övning 1.2 a): the ESP32-S3 factory is still a final, non-copyable implementation.
 */
...
/**
 * @brief Övning 1.2 a): serial() returns a std::unique_ptr holding an ESP32-S3 driver with
...
/**
 * @brief Övning 1.2 b): the stub factory is still a final, non-copyable implementation.
 */
...
/**
 * @brief Övning 1.2 b): serial() ignores the pins and returns a std::unique_ptr holding a new,
...
TEST(StubFactory, CreatesANewStubDriverEachTime)
{
    ...
    EXPECT_EQ(static_cast<std::uint32_t>(byte), 0x7EU);
 ...
 * @return What run() did.
 */
[[nodiscard]] probe::Report watchRun(const bool receives)
{
 ...
 */
[[nodiscard]] Lifetime measureLifetime()
{
    int crashed{};
...
/**
 * @brief Övning 1.3 b): the constructor asks the factory for one serial driver, with the
 *        pins it was given.
 */
...
/**
 * @brief Övning 1.3 a) and c): the std::unique_ptr member destroys the driver exactly once
...
/**
 * @brief Övning 1.3: the constructor is still noexcept, and Logic still cannot be default
...
/**
 * @brief Övning 1.3 d): run() behaves as before: 0, 1, 2, ..., a read after every write, about
 *        100 ms apart.
 */
TEST(Run, SendsAnIncrementingByteEvery100Milliseconds) { EXPECT_THE_LOOP(watchRun(false)); }

/**
 * @brief Övning 1.3 d): run() prints a received byte, and nothing when nothing was received.
 */
...
/**
 * @brief Övning 1.3: main.cpp compiles against your smart-pointer headers.
 */
TEST(Program, Compiles) { EXPECT_TRUE((std::is_same<decltype(exercise1Main()), int>::value)); }
