/**
 * @file The suite's always-on tests: what §4.2 says about owning an object, checked.
 *
 ...
 *       run. This one holds §4.2's claims about std::unique_ptr, each in a form the compiler
 *       or a running program can confirm. The new/delete facts are in L13's suite.
 */
#include <memory>
#include <type_traits>
#include <utility>
...
/**
 * @brief §4.2: a std::unique_ptr cannot be copied, only moved.
 */
...
/**
 * @brief §4.2: moving a std::unique_ptr moves the ownership, and leaves the source empty.
 */
...
/**
 * @brief §4.2: when a std::unique_ptr goes out of scope, the object is destroyed, through
...
/**
 * @brief §4.2: std::make_unique<T>() returns a std::unique_ptr<T>, which converts to a
