/**
 * @file Tests for Övningsset 2: incrementCounter() on an atomic counter, in exercise2.cpp.
 *
 *       The file evolves through the set, and the tests check where it ends up: Övning 2.3's
 *       atomic counter, whose final value is guaranteed. Övning 2.1's unsynchronized version is
...
/**
 * @brief Övning 2.3: one call increments the counter once per iteration, and is noexcept.
 */
...
/**
 * @brief Övning 2.3: two threads of 100000 increments each end at exactly 200000, every time.
 */
...
/**
 * @brief Övning 2.3: eight threads lose no increment either.
 */
...
#ifdef PROGRAM
/**
 * @brief Övning 2.3: the program prints the final value, which is 200000.
 */
