/**
 * @file Tests for Övningsset 7: validateFirmware(), run through std::async, in exercise7.cpp.
 *
...
/**
 * @brief Övning 7.1: no buffer, or an empty one, is not valid firmware. validateFirmware() is
 *        noexcept.
 */
...
/**
 * @brief Övning 7.1: an image is valid unless a byte is 0xFF, wherever that byte is.
 */
...
#ifdef PROGRAM
/**
 * @brief Övning 7.2: the program announces the boot, reports that it is still waiting at most
...
TEST(Program, PollsWhileTheValidationRuns)
{
    ...
    const long long elapsed_ms{l17::elapsedSince(start)};
