/**
 * @file Tests for Övningsset 6: hasNewData(), and the condition variable versions of txThread()
 *       and rxThread(), in exercise6.cpp.
 *
 *       Your main() runs for ten seconds, so it is not run. The tests start the threads
 *       themselves, and stop the receiver the way Övning 6.1 says main() must: the stop flag set
...
/**
 * @brief Everything the two threads share, and the way Övning 6.1 stops them.
 */
struct Channel
{
    SharedMem shared{};
 ...
 */
[[nodiscard]] std::uint32_t lineCount(const std::string& output)
{
...
/**
 * @brief Övning 6.1: hasNewData() is true when there is new data or the stop flag is set, and
 *        is noexcept.
 */
...
/**
 * @brief Övning 6.1: a waiting receiver wakes for each value sent, prints it once, and clears
 *        newData.
 */
TEST(Receiver, WakesForEachValueSent)
{
    ...
    const std::string output{captureOutput(
        [&]
        {
            ...
            std::this_thread::sleep_for(l17::Milliseconds{20});
            channel.send(41U);
            rxConsumedTheFirst =
                l17::waitUntil([&] { return channel.consumed(); }, l17::Milliseconds{1000});
            channel.send(42U);
            rxConsumedTheSecond =
                l17::waitUntil([&] { return channel.consumed(); }, l17::Milliseconds{1000});
            channel.requestStop();
            rxStoppedWhenAsked =
                l17::joinWithin(rx, l17::Milliseconds{1000}, [&] { channel.nudge(); });
        })};
...
/**
 * @brief Övning 6.1: a receiver waiting with no data wakes and returns when stopped, every time.
    ...
    for (int i{}; i < attempts; ++i)
    {
        ...
        output += captureOutput(
            [&]
            {
                ...
                channel.requestStop();
                if (l17::joinWithin(rx, l17::Milliseconds{1000}, [&] { channel.nudge(); }))
                {
...
/**
 * @brief Övning 6.1: the transmitter notifies the receiver, which prints what was sent at once
    ...
    const std::string output{captureOutput(
        [&]
        {
            ...
            std::thread tx{txThread, std::ref(channel.shared), std::ref(channel.mutex),
                           std::ref(channel.cv), std::cref(channel.stop)};
            rxConsumedWhatTxSent = l17::waitUntil(
                [&]
                {
                ...
                l17::Milliseconds{900});
            channel.requestStop();
            txStoppedInTime = l17::joinWithin(tx, l17::Milliseconds{3000});
            rxStoppedWhenAsked =
                l17::joinWithin(rx, l17::Milliseconds{1000}, [&] { channel.nudge(); });
        })};
