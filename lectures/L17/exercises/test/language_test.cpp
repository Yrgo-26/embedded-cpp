/**
 * @file The suite's always-on tests: claims §6.1 makes about threads, mutexes, atomics,
...
/**
 * @brief §6.1.2: std::thread copies its arguments, and std::ref is how a thread gets a
...
/**
 * @brief §6.1.2: a std::thread can be moved but not copied, and once joined it is no longer
 *        joinable.
 */
...
/**
 * @brief §6.1.6: increments of an atomic counter from several threads are never lost.
 */
TEST(Language, AtomicIncrementsSumExactly)
{
    constexpr std::uint32_t threadCount{4U};
...
/**
 * @brief §6.1.5: a lock guard unlocks its mutex when it goes out of scope, and while it
...
/**
 * @brief §6.1.6: a mutex protects several related variables together, which an atomic per
...
/**
 * @brief §6.1.7: std::launch::async runs the task in a new thread, and
...
TEST(Language, TheLaunchPolicyDecidesWhichThreadRunsTheTask)
{
    ...
    EXPECT_TRUE(std::future_status::deferred == deferredFuture.wait_for(l17::Milliseconds{0}));
    EXPECT_TRUE(std::this_thread::get_id() != asyncFuture.get());
    EXPECT_TRUE(std::this_thread::get_id() == deferredFuture.get());
}

/**
 * @brief §6.1.7: a future holds the task's return value, and wait_for() reports when it is
 *        ready.
 */
TEST(Language, AFutureDeliversTheResult)
{
    ...
    const auto beforeRelease = future.wait_for(l17::Milliseconds{10});
    release.set_value();
    const auto afterRelease = future.wait_for(l17::Milliseconds{5000});
...
/**
 * @brief §6.1.8: a thread waiting on a condition variable with a predicate wakes when the
...
TEST(Language, AConditionVariableWakesAWaitingThread)
{
    ...
    cv.notify_one();
    EXPECT_TRUE(l17::joinWithin(waiter, l17::Milliseconds{5000}));
    EXPECT_TRUE(woke);
}
