/**
 * @file Tests for Övningsset 3: SharedMem, txThread() and rxThread(), in exercise3.cpp.
 *
...
/**
 * @brief Övning 3.1: SharedMem holds a 16-bit unsigned value and a flag.
 */
...
/**
 * @brief Övning 3.3: the receiver prints new data, and clears newData once it has consumed it.
 */
TEST(Receiver, ConsumesAndPrintsNewData)
{
    ...
    const std::string output{captureOutput(
        [&]
        {
            ...
            rxClearedNewData = l17::waitUntil(
                [&]
                {
                    std::lock_guard<std::mutex> lock{mutex};
                    return !shared.newData;
                },
                l17::Milliseconds{1000});
            stop.store(true);
            rxStoppedInTime = l17::joinWithin(rx, l17::Milliseconds{1000});
        })};
...
/**
 * @brief Övning 3.3: without new data, the receiver prints nothing.
 */
TEST(Receiver, PrintsNothingWithoutNewData)
{
    ...
    const std::string output{captureOutput(
        ...
        {
            std::thread rx{rxThread, std::ref(shared), std::ref(mutex), std::cref(stop)};
            std::this_thread::sleep_for(l17::Milliseconds{250});
            stop.store(true);
            rxStoppedInTime = l17::joinWithin(rx, l17::Milliseconds{1000});
        })};
...
/**
 * @brief Övning 3.2: the transmitter publishes a value and sets newData, then, a transmission
...
TEST(Transmitter, PublishesIncrementingValues)
{
    ...
    txSentAValue = l17::waitUntil(
        ...
        l17::Milliseconds{1000});
    txSentTheNextValue = l17::waitUntil(
        [&]
        {
            std::lock_guard<std::mutex> lock{mutex};
            second = shared.data;
    ...
    EXPECT_EQ(static_cast<std::uint32_t>(second), static_cast<std::uint32_t>(first) + 1U);
}

/**
 * @brief Övning 3.4: with both threads running, what the transmitter sends the receiver prints,
 *        exactly once.
 */
