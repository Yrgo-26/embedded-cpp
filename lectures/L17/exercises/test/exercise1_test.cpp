/**
 * @file Tests for Övningsset 1: workerThread() with an atomic stop flag, in exercise1.cpp.
 *
 *       The file evolves through the set, and the tests check where it ends up: Övning 1.3's
 ...
 */
[[nodiscard]] std::size_t countWorkerLines(const std::string& output, bool& everyLineIsTheMessage)
{
...
/**
 * @brief Övning 1.3: the worker prints its message and stops soon after the flag is set.
 */
TEST(Worker, StopsWhenTheFlagIsSet)
{
    ...
    const std::string output{captureOutput(
        [&]
        {
            ...
            std::this_thread::sleep_for(l17::Milliseconds{30});
            stop.store(true);
            stoppedWithinASecondOfTheFlag = l17::joinWithin(worker, l17::Milliseconds{1000});
        })};
...
/**
 * @brief Övning 1.3: the worker prints once every printSpeed_ms, so it cannot print more often
...
TEST(Worker, PrintsNoMoreOftenThanItsInterval)
{
    ...
    const std::string output{captureOutput(
        [&]
        {
            ...
            std::this_thread::sleep_for(l17::Milliseconds{200});
            stop.store(true);
            worker.join();
            elapsed_ms = l17::elapsedSince(start);
        })};
...
/**
 * @brief Övning 1.3: the worker runs as long as the flag is false, so with the flag already
