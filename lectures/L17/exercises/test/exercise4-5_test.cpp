/**
 * @file Tests for Övningsset 4 och 5: driver::counter::Interface, the thread-safe
...
namespace
{
/** True if T has a setInitialized(bool) method, which Övning 5.2 adds. */
 ...
 */
void incrementConcurrently(Interface& counter, const std::uint32_t threadCount,
                           const std::uint32_t iterations)
{
    std::vector<std::thread> threads{};
    for (std::uint32_t i{}; i < threadCount; ++i)
    {
...
/**
 * @brief Övningsset 5 on a stub that has setInitialized(): an uninitialized stub ignores
...
void checkInitializationGuards()
{
    ...
    {
        throw std::runtime_error{"Övning 5.2: driver::counter::Stub has no "
...
/**
 * @brief Övning 4.1: the interface is abstract, has a virtual destructor, and declares the
...
/**
 * @brief Övning 4.2: the stub implements the interface, is final, cannot be copied or moved,
...
/**
 * @brief Övning 4.2: a new stub is initialized and at zero, counts, and resets.
 */
...
/**
 * @brief Övning 4.2: eight threads incrementing at once lose no increment.
 */
...
/**
 * @brief Övning 4.2: a reader calling value() while others increment sees the count only ever
...
/**
 * @brief Övning 4.2: reset() from one thread while others increment leaves a count no larger
...
/**
 * @brief Övningsset 5: the initialization flag guards increment(), value() and reset().
 */
...
/**
 * @brief Övning 4.3: counterThread() calls increment() once per iteration, and is noexcept.
 */
...
#ifdef PROGRAM
/**
 * @brief Övning 4.3: the program's two threads, of 100 and 200 iterations, end at 300.
 */
