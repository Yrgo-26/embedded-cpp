/**
 * @file Tests for Övningsset 2: the software timer in exercise2/driver/timer.hpp, and, once
 ...
 */
[[nodiscard]] std::uint32_t timeoutsAfter(driver::Timer& timer, const std::uint32_t ticks) noexcept
{
...
/**
 * @brief Övning 2.1 b) and c): a stopped timer prints its creation, then stops and is destroyed.
 */
...
/**
 * @brief Övning 2.1 b): a timer created running calls start() from its constructor.
 */
...
/**
 * @brief Övning 2.1 d): the timeout is reported back, from a const timer too.
 */
TEST(Timer, ReportsItsTimeout)
{
    suppressOutput(
        []
        {
            ...
            EXPECT_EQ(static_cast<std::uint32_t>(timer.timeout_ms()), 250U);
...
/**
 * @brief Övning 2.1 d): start(), stop() and toggle() change the running state, and say so.
 */
...
/**
 * @brief Övning 2.1 d): a running timer times out on exactly its timeout-th tick, and not
 *        before.
 */
...
/**
 * @brief Övning 2.1 d): hasTimedOut() resets the counter, so the next timeout is a whole
...
/**
 * @brief Övning 2.1 d): a stopped timer does not count.
 */
...
/**
 * @brief Övning 2.1 e): the program prints exactly the example output, five timeouts in 5000
