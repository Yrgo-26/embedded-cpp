/**
 * @file Tests for Övningsset 4: the clear and toggle templates, in an anonymous namespace in
 *       exercise4/main.cpp.
 *
...
/**
 * @brief Övning 4.1: clearing bit 2 of 0xFF gives 0b11111011, as in the expected output.
 */
...
/**
 * @brief Övning 4.1: clearing a bit that is already clear, or any bit, touches no other bit.
 */
...
/**
 * @brief Övning 4.1: the template works for any integral register width, up to bit 63.
 */
...
/**
 * @brief Övning 4.1: clear() is noexcept.
 */
...
/**
 * @brief Övning 4.2: toggling bits 0, 2, 4 and 6 of 0b11111011 gives 0b10101110.
 */
TEST(Toggle, TogglesEveryBitInThePack)
{
    std::uint8_t reg{0xFBU};
    toggle(reg, 0U, 2U, 4U, 6U);
    EXPECT_EQ(static_cast<std::uint32_t>(reg), 0xAEU);
}

/**
 * @brief Övning 4.2: toggling the same bits twice restores the register, and a single bit
 *        is a pack of one.
 */
...
/**
 * @brief Övning 4.2: the template works for wide registers, and is noexcept.
 */
...
#ifdef PROGRAM
/**
 * @brief Övning 4.2: the program prints the expected output, and ends normally.
 */
