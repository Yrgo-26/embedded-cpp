/**
 * @file The suite's always-on tests: claims §1.2 makes about C++, checked.
 *
...
namespace
{
/** A constexpr function, as in §1.6. */
[[nodiscard]] constexpr int add(const int x, const int y) noexcept { return x + y; }

/** One function that promises not to throw, and one that makes no promise (§1.7). */
void quiet() noexcept {}
void loud() {}

/** A default argument, as in §1.8. */
[[nodiscard]] constexpr bool isHex(const std::uint32_t, const bool hex = false) noexcept
{
    return hex;
}

/** Modify the caller's variable through a reference (§1.10). */
constexpr void toggle(bool& state) noexcept { state = !state; }

/** Set one bit in a register of any integral type (§1.12). */
template<typename T>
...
/** Set any number of bits with a fold expression (§1.12). */
 ...
 *
 *        The course writes either `auto x = val` or `type x{val}`, never `auto x{val}`, and
 *        `std::uint32_t` rather than `unsigned`. This test is the one exception to both: what auto
 *        deduces from a braced initializer, and that `10U` is an `unsigned int`, is what it
 *        asserts. Leave the declarations below as they are.
 */
...
TEST(Language, FoldExpressionsSetEveryBitInThePack)
{
    ...
    EXPECT_EQ(static_cast<std::uint32_t>(reg), 0x3EU);
}
 ...
 *        unsigned types, which is why §1.2's isUnsigned trait must not specialize both.
 */
