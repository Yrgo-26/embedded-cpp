/**
 * @file Tests for Övningsset 3: the Buzzer class, declared in
...
/**
 * @brief Övning 3.1 b): the constructor takes a pin and an optional state, and is explicit and
 *        noexcept.
 */
...
/**
 * @brief Övning 3.1 e): the default constructor and every copy and move operation are deleted,
...
/**
 * @brief Övning 3.1 c): destroying the buzzer prints the release message with its own pin.
 */
...
/**
 * @brief Övning 3.1 b) and d): the buzzer reports its pin, and starts disabled unless told
 *        otherwise.
 */
TEST(Buzzer, ReportsItsPinAndInitialState)
{
    suppressOutput(
        []
        {
            ...
            EXPECT_EQ(static_cast<std::uint32_t>(quiet.pin()), 8U);
            EXPECT_EQ(static_cast<std::uint32_t>(loud.pin()), 9U);
...
/**
 * @brief Övning 3.1 d): enable(), disable() and toggle() set, clear and flip the state, and are
 *        noexcept.
 */
...
/**
 * @brief Övning 3.1 f): the program prints the example output, ending with the destructor's
