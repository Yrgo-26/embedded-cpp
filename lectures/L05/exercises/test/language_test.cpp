...
namespace
{
/** A struct and a class that differ only in the keyword (§2.1, Terminology). */
struct PlainStruct
{
    int value;
};
...
/** The Led of §2.2, first with an implicit constructor and then with an explicit one. */
...
/** The function §2.2 passes an LED to by value. */
...
/** A class that counts its instances with static members (§2.2, static members). */
class Gpio final
{
public:
...
/** A class holding a const member, as §2.2's Gpio holds myPin and myDirection. */
struct ConstMember
{
...
/** The enumeration class of §2.1, step 2. */
...
/**
 * @brief §2.1: the only formal difference between a struct and a class is the default
...
/**
 * @brief §2.2: a constructor that is not explicit lets a pin number turn into an LED on its
...
/**
 * @brief §2.2: marking the constructor explicit stops the conversion, and nothing else;
...
TEST(Language, AnExplicitConstructorDoesNotConvert)
{
    ...
    EXPECT_EQ(static_cast<std::uint32_t>(led.pin()), 8U);
}

/**
 * @brief §2.2: a static method is called through the class, and a static member counts
 *        the instances every object shares.
 */
TEST(Language, StaticMembersBelongToTheClass)
{
    EXPECT_EQ(static_cast<std::uint32_t>(Gpio::instanceCount()), 0U);
    {
        const Gpio led{13U};
        const Gpio button{2U};
        EXPECT_EQ(static_cast<std::uint32_t>(Gpio::instanceCount()), 2U);
    }
    EXPECT_EQ(static_cast<std::uint32_t>(Gpio::instanceCount()), 0U);
}

/**
 * @brief §2.2: a static constexpr member is a compile-time constant that needs no
...
/**
 * @brief §2.2: a const member can be copied into a new object, but an existing object
...
/**
 * @brief §2.2: std::move() is what selects the move constructor. A class that declares a
...
/**
 * @brief §2.1, step 2: an enumeration class has the underlying type it is given, does not
