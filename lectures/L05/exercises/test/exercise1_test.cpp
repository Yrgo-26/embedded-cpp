/**
 * @file Tests for Övningsset 1: the Led class in exercise1/include/driver/gpio/led.hpp, the
...
/**
 * @brief Övning 1.1 b): the constructor takes a pin and an optional state, and is explicit, so
...
/**
 * @brief Övning 1.1 c): the destructor is noexcept and defaulted. A defaulted destructor of a
...
/**
 * @brief Övning 1.1 e): the default constructor and every copy and move operation are deleted,
...
/**
 * @brief Övning 1.1 b): the LED starts off unless told otherwise.
 */
...
/**
 * @brief Övning 1.1 d): on(), off() and toggle() set, clear and flip the state, and are
 *        noexcept.
 */
...
/**
 * @brief Övning 1.2 b) and c): the constructor takes a pin, is explicit and noexcept, and the
...
/**
 * @brief Övning 1.2 e): the default constructor and every copy and move operation are deleted,
 *        and the class is final, as declared.
 */
...
/**
 * @brief Övning 1.2 b) and d): a new button reports its pin and starts released.
 */
TEST(Button, ReportsItsPinAndStartsReleased)
{
    const Button button{2U};
    EXPECT_EQ(static_cast<std::uint32_t>(button.pin()), 2U);
...
/**
 * @brief Övning 1.2 d): setPressed() updates the state, both ways, and is noexcept.
 */
...
/**
 * @brief Övning 1.1 f) och 1.2 f): the program prints both example outputs, in order.
 */
