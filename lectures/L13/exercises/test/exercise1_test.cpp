/**
 * @file Tests for Övningsset 1: driver::serial::Interface, driver::serial::Stub and
...
/**
 * @brief Övning 1.1: the interface is abstract, and its destructor is virtual and noexcept.
 */
...
/**
 * @brief Övning 1.1 b): bool isInitialized() const noexcept.
 */
...
/**
 * @brief Övning 1.1 c): void write(std::uint8_t) noexcept.
 */
...
/**
 * @brief Övning 1.1 d): bool read(std::uint8_t&) noexcept.
 *
...
/**
 * @brief Övning 1.2: the stub is a final implementation of the interface, created without
...
/**
 * @brief Övning 1.2 e): the stub can be neither copied nor moved.
 */
...
/**
 * @brief Övning 1.2 b) and d): a new stub is initialized, and has nothing to read.
 */
...
/**
 * @brief Övning 1.2 d): a byte written can be read back once, through the interface.
 */
TEST(Stub, ReadsBackTheLastByteWrittenOnce)
{
    ...
    EXPECT_EQ(static_cast<std::uint32_t>(byte), 0x42U);
    EXPECT_FALSE(serial.read(byte));

    serial.write(0x01U);
    serial.write(0xFFU);
    EXPECT_TRUE(serial.read(byte));
    EXPECT_EQ(static_cast<std::uint32_t>(byte), 0xFFU);
    EXPECT_FALSE(serial.read(byte));
}

/**
 * @brief Övning 1.3: the ESP32-S3 driver is a final implementation of the interface.
 */
...
/**
 * @brief Övning 1.3 b): the constructor takes two pins, is noexcept, and is explicit, so
...
/**
 * @brief Övning 1.3 e): no default constructor, and no copy or move.
 */
...
/**
 * @brief Övning 1.3 d): write() prints the byte in hexadecimal and the TX pin, as in the
...
/**
 * @brief Övning 1.3 d): the placeholder driver is always initialized, and never receives.
 */
