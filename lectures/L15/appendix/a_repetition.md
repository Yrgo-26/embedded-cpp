...
* Skapa en stubb-factory `driver::factory::Stub`, som ärver `driver::factory::Interface` och implementerar:
    * `gpio(std::uint8_t pin)`, som skapar och returnerar en ny GPIO-stubb i form av en smart pekare(`std::unique<driver::gpio::Interface>`).
        * Pin-numret används inte i stubben: skriv `(void) (pin);`.
    * `serial()`, som skapar och returnerar en ny serial-stubb i form av en smart pekare (`std::unique_ptr<driver::serial::Interface>`).

Viktigt:
* Använd `std::make_unique()` från `<memory>` för att skapa instanserna.
* När smarta pekare används behöver vi inte frigöra resurserna manuellt - detta görs av destruktorn när pekaren går ur scope.
...
struct Shared
{
    ...
    std::unique_ptr<driver::serial::Interface> serial;
