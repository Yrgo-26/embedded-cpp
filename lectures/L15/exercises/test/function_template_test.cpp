/**
 * @file Tests for Övningsset 1: the clear and toggle function templates, in
 ...
 */
[[nodiscard]] constexpr std::uint8_t atCompileTime() noexcept
{
...
/**
 * @brief Övning 1.1 b): clearing bit 2 of 0xFF gives 0b11111011, as in the expected output.
 */
TEST(Clear, ClearsTheBit)
{
    std::uint8_t reg{0xFFU};
    clear(reg, 2U);
    EXPECT_EQ(static_cast<std::uint32_t>(reg), 0xFBU);
}

/**
 * @brief Övning 1.1 b): clearing a bit touches no other bit, and a clear bit stays clear.
 */
TEST(Clear, LeavesTheOtherBitsAlone)
{
    std::uint8_t reg{0x0FU};
    clear(reg, 7U);
    EXPECT_EQ(static_cast<std::uint32_t>(reg), 0x0FU);
    clear(reg, 0U);
    EXPECT_EQ(static_cast<std::uint32_t>(reg), 0x0EU);
}

/**
 * @brief Övning 1.1: the template works for every integral register width, up to bit 63.
 */
TEST(Clear, WorksForEveryWidth)
{
    std::uint16_t half{0xFFFFU};
    clear(half, 15U);
    EXPECT_EQ(static_cast<std::uint32_t>(half), 0x7FFFU);
...
/**
 * @brief Övning 1.1: clear() is noexcept, as its declaration says.
 */
...
/**
 * @brief Övning 1.2 d): toggling bits 0 to 3 of 0b11111011 gives 0b11110100, as in the expected
 *        output.
 */
TEST(Toggle, TogglesEveryBitInThePack)
{
    ...
    toggle(reg, 0U, 1U, 2U, 3U);
    EXPECT_EQ(static_cast<std::uint32_t>(reg), 0xF4U);
}

/**
 * @brief Övning 1.2 d): toggling the same bits twice restores the register, and one bit is a
 *        pack of one.
 */
TEST(Toggle, TogglingTwiceIsNoChange)
{
    ...
    toggle(reg, 1U, 3U, 7U);
    toggle(reg, 1U, 3U, 7U);
    EXPECT_EQ(static_cast<std::uint32_t>(reg), 0x5AU);
    toggle(reg, 0U);
    EXPECT_EQ(static_cast<std::uint32_t>(reg), 0x5BU);
}

/**
 * @brief Övning 1.2: the template works for every integral register width, up to bit 63.
 */
TEST(Toggle, WorksForEveryWidth)
{
    std::uint16_t half{};
    toggle(half, 0U, 15U);
    EXPECT_EQ(static_cast<std::uint32_t>(half), 0x8001U);
...
/**
 * @brief Övning 1.2: toggle() is noexcept, as its declaration says.
 */
...
TEST(Templates, AreConstexpr)
{
    ...
    EXPECT_EQ(static_cast<std::uint32_t>(atCompileTime()), 0xF4U);
}

#ifdef PROGRAM
/**
 * @brief Övning 1.2 e): the program prints the expected output.
 */
