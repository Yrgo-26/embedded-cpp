/**
 * @file Tests for Övningsset 1: the Timer class template and its STM32 specialization in
 ...
 */
template<Type T>
[[nodiscard]] std::uint32_t timeoutsAfter(Timer<T>& timer, const std::uint32_t ticks) noexcept
{
 ...
 */
[[nodiscard]] bool endsWith(const std::string& text, const std::string& suffix) noexcept
{
...
void expectLifetimePrintsCaptured(const std::string& name)
{
    ...
    EXPECT_EQ(static_cast<std::uint32_t>(timer->timeout_ms()), 1000U);
...
void expectLifetimePrints(const std::string& name)
{
    ...
    suppressOutput([&] { expectLifetimePrintsCaptured<T>(name); });
}
...
{
    // The outer capture keeps a timer destroyed by a failed expectation out of the report.
    suppressOutput([&] { expectZeroTimeoutRejectedCaptured<T>(name); });
}
...
void expectCounting()
{
    suppressOutput(
        []
        {
            ...
            Timer<T> slow{60000U};
            EXPECT_EQ(static_cast<std::uint32_t>(slow.timeout_ms()), 60000U);
            slow.start();
...
/**
 * @brief Övning 1.1: Type is an enumeration class with an 8-bit underlying type, and a timer
