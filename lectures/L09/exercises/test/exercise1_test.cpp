/**
 * @file Tests for Övningsset 1: the serial interface in include/driver/serial/interface.hpp.
 *
 ...
 *       method with the exact signature Övning 1.1 specifies, so it only compiles if your
...
/**
 * @brief Övning 1.1: every method except the destructor is pure virtual, so the interface is
...
/**
 * @brief Övning 1.1 a): the destructor is virtual, so deleting an implementation through a
...
/**
 * @brief Övning 1.1 b): isInitialized() returns a bool, is const and noexcept, and is
...
/**
 * @brief Övning 1.1 c): write() takes one std::uint8_t, returns nothing, is noexcept, and is
 *        dispatched to the implementation.
 */
TEST(Interface, WriteTransmitsOneByte)
{
    ...
    EXPECT_EQ(static_cast<std::uint32_t>(serial.myLastByte), 0x41U);
}

/**
 * @brief Övning 1.1 d): read() takes a reference to the byte it stores into, returns a bool,
 *        is noexcept, and its result may be discarded.
 *
 *        Whether read() is [[nodiscard]], and isInitialized() is, cannot be tested here: as
 *        §3.2 notes, GCC does not warn when the result of a virtual call is discarded, so
...
TEST(Interface, ReadStoresIntoAReference)
{
    ...
    interface.read(byte);
    EXPECT_EQ(static_cast<std::uint32_t>(byte), 0x5AU);
}
