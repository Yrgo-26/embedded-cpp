/**
 * @file Tests for Övningsset 2: the serial stub, declared in include/driver/serial/stub.hpp
...
/**
 * @brief Övning 2.1: the stub inherits the interface publicly and is marked final.
 */
...
/**
 * @brief Övning 2.1 c): the stub cannot be copied or moved, neither constructed nor assigned.
 */
...
/**
 * @brief Övning 2.1 b): a new stub is initialized and has nothing to read.
 */
...
/**
 * @brief Övning 2.1 d): a written byte can be read back once, and then there is nothing more
 *        to read.
 */
TEST(Stub, ReadsBackTheByteWrittenOnce)
{
    ...
    EXPECT_EQ(static_cast<std::uint32_t>(byte), 0x41U);
    EXPECT_FALSE(stub.read(byte));
}

/**
 * @brief Övning 2.1 d): the stub keeps the most recently transmitted byte, not the first.
 */
...
/**
 * @brief Övning 2.1 d): read() copies into its argument only when it returns true.
 */
...
/**
 * @brief Övning 2.1 d) and e): an uninitialized stub reports so, ignores writes, and has nothing
...
/**
 * @brief Övning 2.1 d): an uninitialized stub refuses to read even a byte it already holds.
 */
...
/**
 * @brief Övning 2.1 d): through a reference to the interface, the stub behaves the same, and
