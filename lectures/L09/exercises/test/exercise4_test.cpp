/**
 * @file Tests for Övningsset 4: sendMessage() and the program, in source/main.cpp.
 *
...
namespace
{
/** The message Övning 4.1 a) asks sendMessage() to transmit. */
...
public:
    ...
    std::uint32_t myWrites{};
};
} // namespace

/**
 * @brief Övning 4.1 a): sendMessage() transmits exactly the message, one byte per write(), with
...
TEST(SendMessage, TransmitsTheMessageOneByteAtATime)
{
    ...
    EXPECT_EQ(recorder.myWrites, static_cast<std::uint32_t>(expectedMessage.size()));
}

/**
 * @brief Övning 4.1 c): after sendMessage(), the stub holds the last byte of the message, '!'.
 */
...
#ifdef PROGRAM
/**
 * @brief Övning 4.1 b): the program prints the message through the console driver, as the first
