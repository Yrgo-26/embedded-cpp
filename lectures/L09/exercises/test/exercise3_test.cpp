/**
 * @file Tests for Övningsset 3: the console driver, a singleton, declared in
...
/**
 * @brief Övning 3.1: the console inherits the interface publicly and is marked final.
 */
...
/**
 * @brief Övning 3.1 a) and b): nobody else can create or destroy a console, because its
...
/**
 * @brief Övning 3.1 c): the one instance cannot be copied or moved, neither constructed nor
 *        assigned.
 */
...
/**
 * @brief Övning 3.1 d): instance() is static and noexcept, returns a reference to the interface,
...
/**
 * @brief Övning 3.1 e): the console is always initialized, and cannot be read from.
 */
TEST(Console, IsAlwaysInitializedAndWriteOnly)
{
    ...
    suppressOutput([&] { console.write('A'); });
    EXPECT_FALSE(console.read(byte));
}

/**
 * @brief Övning 3.1 e): write() prints each byte as the character it is, and nothing else.
 */
